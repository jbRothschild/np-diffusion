...
def params(other = []):
    ...
    nu = 0.002 # nu = dudx
    save_time = 300.
    model_var = [5000] #Model variant. In this model: [hole number]. ##Note these can change depending on what we're doing
    ...
    return vis, dx, dy, dz, total_time, dt, nu, save_time, model_var
...
def create_flow_location(load_dir, data_dir, filename, other = []):
    """
...
def create_diffusion_location(load_dir, data_dir, filename, other = []):
    """
...
def model(load_dir, data_dir, model_var):
    ...
    """
    tic = time.time()
    SL = create_source_location(load_dir, data_dir, 'UT16-T-stack3-Sept10_iso_500000gaps.tif', other = [ model_var[0] ])
...
def update_diff(holes_location, source_location, data_dir):
